...
public class ObjectPlacer : MonoBehaviour
{
    ...
    {
        Vector2 screeCenter = _camera.ViewportToScreenPoint(new Vector2(0.5f, 0.5f));

        var ray = _camera.ScreenPointToRay(screeCenter);

        if(Physics.Raycast(ray, out RaycastHit raycastHit))
        {
            SetObjectPosition(raycastHit.point);
        }
        else if(_arRaycastManager.Raycast(screeCenter, _hits, TrackableType.PlaneWithinPolygon))
        {
